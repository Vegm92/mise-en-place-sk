...
- Issue #391: `login`, `signup`, `forgot-password`, and `waitlist` each reimplemented the same honeypot-check-then-rate-limit-then-handle shape, with the rate-limit policy (which keys, which caps) buried in each route. This wraps that shape: reject a filled `_hp` field before touching any limiter, then check rules IP-scope first regardless of the order the caller lists them in. Issue #510 made this short-circuit at the first failing rule instead of running every rule: a route keying on both IP and email (login, forgot-password) no longer consumes the email-scoped bucket once the IP-scoped one has already blocked the attempt — `tests/public-form-action.test.ts` pins this. `onboarding` deliberately stays out: it is session-gated, not public, and has neither a honeypot nor a rate limit to share.
...
- Issue #844: an optional `schema` option (a `valibot`/Standard-Schema-v1 schema) runs after rate limiting and before the handler — `parseForm()` builds a plain object from the `FormData` via `formToRecord()` (first value per key, matching `FormData.get()`'s semantics rather than `Object.fromEntries()`'s last-value-wins, so a repeated field name can't smuggle a second, differently-typed value past the schema) and `v.safeParse()`s it; a failure returns `fail(422, { error: 'invalid' })` — the same shape the honeypot check already used, so no new i18n key was needed on the routes that go through `publicFormAction` (`signup`, `login`, `forgot-password`, `waitlist`). On success the handler receives `ctx.data`, typed from `v.InferOutput<TSchema>`, instead of casting `form.get(...)` itself. `formToRecord`/`parseForm` are exported so `reset-password` (the one public auth route with no rate limit or Turnstile, so it doesn't go through `publicFormAction` at all) can call `parseForm` directly; its own schema-parse failure deliberately reuses the already-mapped `expired` code rather than introducing an unmapped `reset.err.invalid` (`src/lib/i18n-messages.ts` was another workstream's surface while #844 was in flight). `scripts/lint-invariants.mjs`'s `form-get-cast` gate bans a route from reverting to a raw `as` cast.
