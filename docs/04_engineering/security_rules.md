...
**`function runDetached` / `class TenantScopeReleasedError` / `function guardReserved`**

- Issue #1073 (prerequisite of the #975 role cutover). The reserved connection is released in a `finally` the moment `fn` settles, but `AsyncLocalStorage` keeps the store alive in every promise `fn` started and did not await — a fire-and-forget `trackEvent()`, the webhook's `recordAccountEvent(evt).catch(...)`, `void maybeSendQuotaWarning(rid)` — so that work could issue its query on a connection already back in the pool. `postgres.js` does not stop this: a reserved handle keeps sending to its physical connection after `release()`. Today the explicit `forTenant()` predicates still bound the result and it is only a connection-stability bug; under the `mep_runtime` role the same query would be filtered by whatever `app.restaurant_id` the connection's next borrower set — a cross-tenant read or write.
- `guardReserved` marks the context `released` before the GUCs are cleared and patches the reserved handle's `unsafe` (the only entry point `drizzle-orm/postgres-js` uses, including `db.execute`) and the `begin` shim so a query or transaction attempted after that point throws `TenantScopeReleasedError`. It throws in every environment, production included, and reports to Sentry: the only alternative to refusing is silently routing the query to a recycled or unscoped connection, which is exactly the bug. A refused query surfaces as a rejection of the detached promise, which every sanctioned fire-and-forget site already catches; an uncaught one is a missed site and should crash loudly rather than leak. `clearGucs` deliberately bypasses the patch (tagged-template call, not `unsafe`) — it is the one statement that must run after the mark.
- `runDetached(restaurantId, fn)` is the sanctioned shape for background work that may outlive the scope it was started from: inside an active scope it reserves a connection of its own — `tenant` mode for the given restaurant, `admin` when `null` — so the work completes on that connection regardless of when the caller's scope exits; outside any scope it just runs `fn()` on the pooled client, which is already independent of any reservation. Callers (`events.ts`, `invoice-save.ts`) import it from `tenant-context.ts` rather than through `db.ts`: dozens of unit tests mock `$lib/server/db` with a bare `{ db }` object, and a fire-and-forget path must not turn a missing mock into a synchronous `TypeError` inside the save path. The WhatsApp webhook wraps `recordAccountEvent` in `runAsSystem()` directly instead — that route is already in ADR-030's system-context table and the table is a global, tenant-less one — so the intent is explicit at the site.
- `runDetached` reserves a second pool connection for the length of the detached work, so a call site pays two extra round trips (`set_config` on entry, the reset on exit) per event. Cheap next to the request, but it is not free: do not wrap awaited, in-scope work in it.
- Not changed here (#975 will need it): worker jobs that run outside any context (`src/worker.ts`'s extraction handlers) hit the `!als.getStore()` branch and keep writing on the pooled client exactly as before this fix.
