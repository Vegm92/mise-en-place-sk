...
- The tenant's own fiscal identity — `legal_name` (razón social), `cif_nif`, `fiscal_address`, `phone` on `restaurants` (issue #905, task 1; `phone` added by issue #918). `cif_nif`/legal/trade names exist so extraction can tell the *emisor* from the *receptor* on a scanned document by matching the receiver's tax id against the restaurant's own, instead of guessing from names that vary between razón social, nombre comercial and an individual's name; `phone` is the same idea one signal further — see `resolveInvoiceParties`' contact fallback in `invoice_extraction.md`.
...
- Owner-only, same trust level as the rename: the tax id is what will later decide whether a document belongs to this tenant. This is also the *only* place `restaurants.phone` is overwritten unconditionally — an explicit, deliberate edit is the one case allowed to replace whatever `saveReviewedInvoice` filled in from an invoice (issue #918).
...
- Copy went through `$t`/`$ti` against `src/lib/i18n.ts`'s `waitlist.*` namespace (issue #407) instead of a page-local `copy` object; the language toggle is an `<a href>` to the same page in the other locale, not a button — a crawler cannot follow a click handler, so before that change the English rendering was unreachable as well as unrendered ([ADR-033](../06_decisions/experience/ADR-033-the-rendered-locale-is-request-state.md)). It still writes the preference to the shared `locale` writable on click (persisted by `initLocale()` to the same `mep-locale` key the rest of the app uses, now mirrored into a cookie so the server can read it), while what the page *renders* comes from the request-scoped locale context. Pricing-tier names/taglines/bullets are read straight from `billing.plan.*` / `TIER_COPY` (`$lib/billing-plans.ts`) — the same source `BillingPlanCard.svelte` renders from — rather than duplicating that copy; only the numbers (`PROVISIONAL_PRICE`) and quotas are supplied locally. `tests/waitlist-provisional-price.test.ts` diffs every rendered string, per locale, against the pre-migration inline object (pinned by commit SHA) to guarantee the move was byte-identical.
...
- Module-level `Map` caches for `Intl.NumberFormat` and `Intl.DateTimeFormat` instances, avoiding repeated expensive object construction on thousands of format calls across list and dashboard renders (~30-50x speedup). Pre-instantiated static instances for standard options (`eurFormatters`, `eurCompactFormatters`, `yoyFormatters`, `integerFormatters`, `oneDecimalFormatters`, `dateFormatters`, `dateShortFormatters`, `monthShortFormatters`) eliminate `JSON.stringify` key generation overhead on hot paths, doubling formatting performance (2.06x speedup). Bounded `Map` caches (`fmtDateCache`, `fmtDateShortCache`, `fmtMonthShortCache`, max 2000 entries each) memoize `fmtDate`, `fmtDateShort`, and `fmtMonthShort` formatted date string results by locale and date input, eliminating redundant `Date` object construction and `Intl.DateTimeFormat.prototype.format(...)` execution across list and dashboard renders (~6.95x speedup).
**`function fmtSize`**
