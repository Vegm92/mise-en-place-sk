...
| `tenant-weekly-digest` | `src/lib/server/tenant-fanout.ts` | One tenant's weekly digest — generate + email + WhatsApp push to opted-in contacts (`sendWeeklyDigest`, #331) | Fanned out by the cron dispatcher, one job per tenant; `batchSize` = `SCHEDULED_FANOUT_CONCURRENCY` (default 5) with `perJobResults`, 2 retries 120 s apart, then dead-letter (ADR-025) |
...
- WhatsApp push (#331): after the email claim succeeds, a second claim (`weekly_digest_whatsapp_week`) guards `pushWeeklyDigestOverWhatsApp` (`whatsapp-digest-push.ts`) the same way — idempotent per tenant per week, and independent of email delivery. It only ever reaches contacts with `whatsapp_contacts.digest_opted_in = true`, re-verified immediately before each send, and its own internal try/catch means a WhatsApp failure — for one contact or the whole tenant — never fails the email or another tenant's job.
...
### `src/extraction-replay.ts`

**`function main`**

- Operator tool, not a job: `pnpm corpus:replay` re-extracts documents already in the corpus with the *current* prompt and prints per-field agreement against the stored baseline (#813, ADR-034). This is the mechanism for telling a prompt improvement from a regression, since it compares two prompt revisions over the same real documents. `--stats` (corpus by prompt version), `--dry-run` (selection only, no model calls), `--export <file>` (anonymized JSONL), `--limit` (default 10), `--restaurant`, `--prompt-version`.
- Replay rows are stored with `run_kind = 'replay'` so they never mix with the live extraction a tenant actually reviewed. Replays spend Gemini quota and are deliberately not recorded in `llm_usage_log`/`monthly_usage` — a replay is the operator's cost, not the tenant's.
