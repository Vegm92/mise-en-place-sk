...
| **Spec is stale** | Code changed behaviour deliberately; doc never updated | Update the doc (feature spec + CODE_NOTES) to match reality; note the change date |
...
| **Intentional-but-undocumented** | A deliberate divergence a reviewer would not guess | Add a comment-free note in CODE_NOTES + an ADR amendment if the *why* is non-obvious |
...
3. Ask: does the current behaviour look deliberate? (Comments are banned, but
   CODE_NOTES, tests, and commit history reveal intent.)
...
- A pointer for the next agent (link to the spec or CODE_NOTES section)
