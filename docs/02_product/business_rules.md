...
| Extraction concurrency | Global cap `MAX_CONCURRENT_EXTRACTIONS` (default 1) enforced by a distributed semaphore; worker `batchSize` follows the cap | `worker.ts`, `rate-limiter.ts` |
