...
accent is no longer `slate` but the ink itself — and by
[#720](https://github.com/Vegm92/mise-en-place-sk/issues/720), which adds the
on-tint column below
**Feature:** experience
**Date:** 2026-08-26
...
> (4.35:1). Neither is fixed here: #720 scoped this change to the accent/slate
> pair only, and `--mep-neg`/`--mep-caution` are the severity ramp, a
> different blast radius. Filed as follow-up; not yet an issue number.
>
...
|---|---|---|---|---|---|
| Alta | `--mep-neg` | 5.98:1 | 4.99:1 | 5.34:1 | **4.16:1 — fails AA, not fixed by #720** |
| Media | `--mep-warn` | 5.38:1 | 4.55:1 | 7.13:1 | 5.19:1 |
| Baja | `--mep-caution` | 5.24:1 | **4.35:1 — fails AA, not fixed by #720** | 10.18:1 | 6.78:1 |
...
current value). Only the `slate` accent's dark `-soft` alpha changed in this
pass, `rgba(111,143,196,0.16)` → `rgba(111,143,196,0.10)`; `--mep-neg` and
`--mep-caution`'s on-tint gaps are documented, not fixed — see the amendment
above.
...
`tests/contrast-tokens.test.ts` (on-tint ratios, added by #720).
...
- [#720](https://github.com/Vegm92/mise-en-place-sk/issues/720) — found that
  ...
  `slate` dark alpha. `--mep-neg` (dark) and `--mep-caution` (light) fail the
  same on-tint check and are not yet fixed — see the amendment above
