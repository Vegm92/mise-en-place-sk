...
**`function monthKey`**
- The one `Date → "YYYY-MM"` conversion (issue #1068), from **UTC** calendar fields — timestamps are UTC, never the host zone. It replaced four copies that disagreed at every month boundary: `revenue-math.ts`'s (UTC) and the local-zone ones in `formatters.ts` (`toMonthStr`), `reports/index.ts` and `server/dates.ts` (`monthKeyStr`), plus four `toISOString().slice(0, 7)` call sites (`alerts.ts`, `llm-quota.ts`, `quota-warning.ts`, `suppliers/[id]`). On a Europe/Madrid host an invoice saved at 00:30 local on the 1st therefore keys to the previous month everywhere, consistently, instead of to one month on `/admin/revenue` and the next on the budgets page and monthly report.
- Lives here rather than in `server/dates.ts` because the `formatters.ts`/`revenue-math.ts` callers are client-importable; `server/dates.ts` and `revenue-math.ts` re-export it so existing import paths keep working. `tests/money-math-convergence.test.ts` sweeps `src/` and fails on a second template or a `toISOString().slice(0, 7)`.
- Scope, addendum (issue #1117): `monthKey` buckets *machine timestamps* (UTC) — invoice/report keys, quotas, revenue. A *user-facing calendar bucket* (the budgets month) instead uses `currentCalendarMonth`/`monthOf(localToday(...))` in `src/lib/server/period-range.ts`, keyed to `APP_TIMEZONE`. Do not mix the two for the same key.
