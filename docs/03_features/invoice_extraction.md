...
- Checks `jobData.storageFingerprint` against the worker's own (`storageFingerprintMismatch`, `env.ts`, #1049) right after claiming the item and before `claimExtractionAllowance` — ahead of any quota claim or Gemini spend, since a storage mismatch means the uploaded file is not reachable from here at all. `markFailed(itemId, 'extract.err.storageMismatch', { field })` names which of `driver`/`bucket`/`path` disagreed; never the values. A job with no fingerprint (queued before this shipped) always passes.
