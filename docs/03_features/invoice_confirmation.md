...
- Seeded from the extraction but editable, so a reviewer can correct Gemini's factura/albarán call; it drives the header grid rather than the server-stored value, so the form reshapes as soon as the select changes.
- An albarán has no payment term, so the due-date cell becomes a `readonly` input carrying the "no aplica" text plus a hidden `due_date=""`. Deliberately still an `<input>`: the visible cell keeps a real `<label for>` target (a `<div>` is not a labelable element, so the association was inert for screen readers) and stays focusable and announced, which `disabled` would prevent. The hidden field means `due_date` is always present in the submission, so the field-change diff never has to distinguish "absent" from "cleared".
...
- `resolveRestaurantTaxIdSignal` (issue #905 task 3) runs beside it: the document's `receiver_nif` is compared with `restaurants.cif_nif` and a difference surfaces post-commit as `restaurant_tax_id_mismatch`. A warning, never a hard fail — the profile is as likely to be the wrong side as the document, and refusing to file an invoice on a tax id read by OCR would lose real documents. It stays silent unless the extracted id is checksum-valid and legible (`taxIdDecidesIdentity`), and it writes nothing: a blank `cif_nif` is left blank rather than filled from a document whose ownership is exactly what is in question — the opposite of the phone rule below, whose worst case is a stale phone number rather than a wrong fiscal identity feeding every later party split.
- `resolveRestaurantPhoneSignal` (issue #918) runs in the same transaction as the supplier upsert: `receiver_phone` fills a blank `restaurants.phone` (`COALESCE`, race-safe) but never overwrites one already on file — that value may be the owner's own edit in Settings, which extraction has no way to tell apart from a stale one. A mismatch against an existing phone is reported outward, not applied, and surfaces post-commit as a `restaurant_phone_mismatch` notification linking back to Settings.
...
- Takes a single `DuplicatePurchaseInput` object rather than positional arguments. Adding `lineDescriptions` had pushed it to eight positionals, four of them adjacent strings — `supplierName`/`restaurantId` and `invoiceDate`/`totalAmount` are each transposable without a type error, and a silent swap there would compare the wrong columns rather than fail. Named fields make that class of mistake impossible at the call site.

**`function linkRelatedDocuments`**
