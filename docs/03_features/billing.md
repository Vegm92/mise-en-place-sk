...
- Price ids come from `env.ts`, the single reader of `STRIPE_PRICE_ID_*` and the legacy `STRIPE_PRICE_ID`, which **trims** them at the point of export (issue #1075): they are pasted out of the Stripe dashboard into a deploy console, where a trailing space or newline survives, is invisible in the dashboard, and turns every exact-match lookup in `tierFromPriceId` into a silent miss. Before #1075 `billing.ts` trimmed its own re-read of `process.env` while `env.ts` exported the raw value, so a padded id worked for checkout and failed in the admin Stripe probe — the two readers disagreed about the same variable. `external-probes.ts` now probes `TIERS.starter.stripePriceId`, the exact id checkout sends, legacy fallback included.
...
- The dedupe does not apply to a **paying subscription** (issue #1075). The webhook and reconcile paths pass a `SubscriptionTierContext` (subscription id, Stripe status, restaurant id) through `subscriptionFields`; when the status is `active` or `past_due` and the price matches nothing, `reportPaidTierMismatch` emits an error-level structured log line (`createLogger('billing')`) and a Sentry `captureException` tagged `op:paid_tier_mismatch` with the subscription id, restaurant id, status and price in `extra` — once per subscription + price id per process (`reportedPaidTierMismatches`), because `syncSubscriptionFromStripe` re-resolves the live subscription on every `/billing` load and a per-write event would be one Sentry event per page view; the log line still marks every resolution. It still returns `starter` rather than throwing: a throw in the webhook would 500 the event, leave the row stale and, on `checkout.session.completed`, leave the tenant with no subscription row at all (which `getAccessState` treats as denied). The tenant keeps a working billing page on starter entitlements until the operator fixes the price id and the next event or reconcile corrects the tier. `trialing`, `canceled`, `paused` and `incomplete` stay on the deduped path — nothing is being billed for the wrong tier there. The confirm page calls `tierFromPriceId` without a context (it has a checkout session line item, not a subscription) and stays deduped.
