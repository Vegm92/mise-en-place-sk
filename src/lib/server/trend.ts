...
export type TrendScale = 'daily' | 'weekly' | 'monthly' | 'yearly' | '7d' | '30d' | '90d';

const VALID_SCALES = new Set<string>(['daily', 'weekly', 'monthly', 'yearly', '7d', '30d', '90d']);
...
export function normalizeTrendScale(raw: string | null): TrendScale {
	return raw && VALID_SCALES.has(raw) ? (raw as TrendScale) : 'monthly';
}

export async function getTrendData(rid: string, scaleParam: string | null) {
	const tdb = forTenant(rid);
	const scale = normalizeTrendScale(scaleParam);
	...
	return { scale, buckets, categories: [...catSet] };
}
