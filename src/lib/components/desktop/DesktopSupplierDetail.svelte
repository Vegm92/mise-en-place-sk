              ...
              <svg width="100%" viewBox="0 0 {VW} 200" style="display:block;overflow:visible;margin-top:12px;">
                {#each monthly as bar, i}
                  ...
                  <text x={bx + 18} y={CB + 14} text-anchor="middle"
                    font-size="10.5" fill="var(--mep-fg-3)">{fmtMonthShort(bar.key, locale.current)}</text>
                {/each}
                {#if chartAvg > 0}
              ...
              <tbody>
                {#each products as p (p.description + '|' + p.unit)}
                  <tr>
                    ...
                    <td class="num" style="font-size:12.5px;">{p.avgPrice != null ? fmtEur(p.avgPrice, locale.current) : '—'}</td>
                    <td class="num" style="font-size:12.5px;">{p.totalSpend != null ? fmtEur(p.totalSpend, locale.current) : '—'}</td>
