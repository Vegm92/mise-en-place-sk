...
<div class="hidden md:flex flex-col gap-3 p-4" style="min-height:0;">
  ...
  <div class="card" style="padding:10px 18px;display:flex;align-items:center;gap:20px;flex-shrink:0;flex-wrap:wrap;" data-coach="dashboard-main">
    <StatusChip
      ...
      value={fmtEurCompact(data.mom.this_month, locale.current)}
      tone={hasBudget && paceDelta > 0 ? 'neg' : 'pos'}
      note={hasBudget ? ti('turno.ribbon.paceNote', { delta: fmtEurSigned(paceDelta, locale.current), day: daysElapsed }) : momNote}
      wide={hasBudget}
    >
      {#snippet chart()}
        {#if hasBudget}
    ...
    <StatusChip
      ...
      value={fmtEurCompact(projectedEom, locale.current)}
      tone={hasBudget && overrun > 0 ? 'neg' : 'pos'}
      note={hasBudget ? ti('turno.ribbon.forecastNote', { delta: fmtEurSigned(overrun, locale.current) }) : t('turno.ribbon.forecastNoBudget')}
    />

    <StatusChip
      label={t('turno.ribbon.review')}
      value={fmtEurCompact(data.review.amount, locale.current)}
      tone={reviewTone}
    ...
    <div style="text-align:right;">
      ...
      <div class="num title-lg" style="line-height:1.15;">
        {fmtEurCompact(stake, locale.current)}
      </div>
    </div>
  </div>
