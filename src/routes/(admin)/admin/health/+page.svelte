...
<div style="padding:28px 32px;max-width:860px;margin:0 auto;display:flex;flex-direction:column;gap:20px;">
  ...
  <div style="display:flex;align-items:center;gap:12px;">
    <h2 style="margin:0;font-size:22px;font-weight:600;color:var(--mep-fg);letter-spacing:-0.3px;">{$t('admin.systemHealth')}</h2>
    <span style="
    ...
    <span style="font-size:12px;color:var(--mep-fg-4);margin-left:auto;">
    ...
    <section>
      ...
      {:else}
        <div class="card" style="padding:14px 16px;display:flex;gap:24px;flex-wrap:wrap;font-size:13px;">
          <div>
            <div style="font-size:11px;color:var(--mep-fg-3);text-transform:uppercase;letter-spacing:0.05em;">{$t('admin.wa.quality')}</div>
          ...
          <div>
            <div style="font-size:11px;color:var(--mep-fg-3);text-transform:uppercase;letter-spacing:0.05em;">{$t('admin.wa.messagingLimit')}</div>
            <div style="font-weight:600;color:var(--mep-fg);">{data.whatsapp.health.messagingLimit ?? 'unknown'}</div>
          </div>
          <div>
            <div style="font-size:11px;color:var(--mep-fg-3);text-transform:uppercase;letter-spacing:0.05em;">{$t('admin.wa.worst30d')}</div>
            ...
            <tbody>
              {#each data.whatsapp.events as evt (evt.id)}
                <tr style="border-bottom:1px solid var(--mep-divider,#e5e5e5);">
                  ...
                  <td style="padding:7px 16px;font-family:monospace;font-size:12px;color:var(--mep-fg);">
      ...
      {#if data.whatsapp.tenants.length > 0}
        <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.07em;color:var(--mep-fg-3);margin:16px 0 10px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          ...
          <tbody>
            {#each data.tableCounts as t}
              <tr style="border-bottom:1px solid var(--mep-divider,#e5e5e5);">
                <td style="padding:7px 16px;font-family:monospace;font-size:12px;color:var(--mep-fg-2);">{t.table}</td>
                <td style="padding:7px 16px;text-align:right;color:var(--mep-fg);" class="num">{t.rows.toLocaleString('en-US')}</td>
              </tr>
            {/each}
          </tbody>
        </table>
