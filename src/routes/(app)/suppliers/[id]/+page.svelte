<script lang="ts">
  ...
  const chartMax = $derived(Math.max(...data.monthly.map(m => m.value), 1));
  const chartAvg = $derived((() => {
    const nonZero = data.monthly.filter(m => m.value > 0);
  ...
  // SVG chart constants
  const CL = 40;   // chart left
  const CW = 620;  // chart area width
  const CH = 140;  // chart bar height
  const CB = 170;  // chart bottom y (bars sit above this)
        ...
        <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:14px;align-items:start;">
          ...
          <div style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div class="card" style="padding:16px 16px 12px;">
              ...
              <svg width="100%" viewBox="0 0 {VW} 200" style="display:block;overflow:visible;margin-top:12px;">
                {#each data.monthly as bar, i}
                  {@const colW = CW / data.monthly.length}
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
              <div class="card" style="padding:14px;">
                ...
                <div style="font-size:11.5px;color:var(--mep-fg-3);margin-top:6px;">{data.invoices.length} facturas</div>
              </div>
              <div class="card" style="padding:14px;">
          ...
          <div style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div class="card" style="padding:0;overflow:hidden;">
              <div style="padding:14px 16px 8px;display:flex;align-items:center;justify-content:space-between;">
                ...
                {#if data.invoices.length > 5}
                  <button style="font-size:12.5px;color:var(--mep-acc);font-weight:500;background:none;border:0;cursor:pointer;padding:0;"
                    ...
                    Ver todas ({data.invoices.length})
                  </button>
                {/if}
              </div>
              ...
              {:else}
                {#each data.invoices.slice(0, 5) as inv (inv.id)}
                  ...
                  <div style="padding:10px 16px;display:flex;align-items:center;gap:10px;
                    ...
                    <span class="{SUPPLIER_BADGE_CLS[badge] ?? 'badge'}" style="font-size:10px;padding:1px 5px;">
