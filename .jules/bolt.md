...
One file per optimization, under `.jules/bolt/`. **Do not edit this file** —
not even to add a line to the list below.
...
nothing else. Adding one index line per run caused the same conflict again
(#1165/#1168): GitHub ignores the `merge=union` rule in `.gitattributes`.

## Adding an entry

Create exactly one new file, `.jules/bolt/<date>-<module>-<what-changed>.md`
(path from the repo root — not a top-level `bolt/` directory), e.g.
`.jules/bolt/2026-09-09-trend-fast-spend-trend-segment-indexing.md`. Touch no
other journal file. A new file cannot conflict with another run's new file.
...
Frozen on 2026-09-24. The full, current list is the `.jules/bolt/` directory
(file names sort by date).
