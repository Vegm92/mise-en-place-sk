...
**Limitación conocida y esperada, luego resuelta:** al no haber subido un archivo real, la vista previa del documento en el detalle del albarán mostraba "404 File not found" — esperado, ya que solo se habían simulado los datos, no el archivo. Se resolvió copiando los dos PDF reales directamente a la ruta de almacenamiento que la app ya esperaba (`uploads/seed/...`), sin tocar ningún dato — la usuaria vio el PDF real y los datos extraídos lado a lado en su navegador.

**Categorías solapadas — decisión tomada al ver el caso real de Viñals Gourmet:** al comprobar que ese proveedor (carnes, pero con una línea de queso) solo podía llevar una categoría, la usuaria confirmó que el diseño de fondo está mal (forzar una única categoría por proveedor), no solo la lista de nombres. Se decidió pasar a un modelo de dos niveles — **Tipo** (Bebidas/Comida/Artículos/Ambas, obligatorio, único, es el filtro real) + **Etiquetas** (Carnes, Lácteos, Pescado, Cava... opcionales, varias a la vez, solo para buscar/informar). Alcance: solo proveedores por ahora, productos se quedan como están. Diseño completo, tabla de migración y qué falta construir en la incidencia **#22** de `INCIDENCIAS_AUDITORIA.md` (actualizada, ya no dice "pendiente de decidir" sino "decidido, pendiente de construir").
...
**Rediseño de Analíticas (`/analytics/spend`), a petición de la usuaria — dos vueltas, esto es el estado final:**
- Quitadas las tarjetas sueltas "Categoría con más gasto" / "Proveedor con más gasto" (un nombre + un euro no le servía de nada).
- **"Top productos"** (antes "Artículos principales por gasto"): cada línea muestra el € y el **% real sobre el gasto total** — antes ese número (que solo existía como ancho de barra, sin imprimirse) era relativo al artículo más caro, no al total, así que al empezar a imprimirlo como "%" el número **mentía**. Corregido separando dos valores: `pct` (ancho de barra, relativo al primero, solo visual) y `pctOfTotal` (el que se imprime, de verdad sobre el total).
...
- **Segunda vuelta — "Bebida/Comida/Otros" no le convencía y pidió rehacerlo entero:** dijo que ese desglose es de lo más relevante para ella "como ejecutiva", más que "Top productos", y que quería un sub-desglose al hacer clic (ej. Bebidas → Vinos y Cavas, Café... → qué vinos tiene, de un vistazo). Construido como árbol de 3 niveles con una sola consulta (`type_breakdown` en el server: Tipo → categoría de producto → producto, con % siempre sobre el gasto total para que sea comparable entre niveles) y un desplegable de acordeón en el cliente (clic en Bebida/Comida/Otros abre las categorías; clic en una categoría abre los productos) — sin ir al servidor otra vez al expandir, todo ya cargado.
- **Colocación final, en dos pasos según fue pidiendo la usuaria:** primero "Bebida/Comida/Otros" pasó a ser el panel más prominente, arriba del todo (antes que "Gasto en el tiempo"); después pidió que "Top productos" fuera a su derecha siguiendo la rejilla — quedó: fila 1 = Bebida/Comida/Otros (ancho) + Top productos (estrecho, top 5); fila 2 = Gasto en el tiempo (ancho) + Proveedores más recurrentes (estrecho), sin cambios en esta última fila.
- Todo esto se replicó también en la versión móvil (`MobileAnalyticsSpend.svelte`), con el mismo acordeón, para no romperla.
