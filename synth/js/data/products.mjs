...
].map(([desc, unit, min, max]) => ({ desc, unit, min, max, rate: 0.04 }));
...
].map(([desc, unit, min, max]) => ({ desc, unit, min, max, rate: 0.10 }));
...
].map(([desc, unit, min, max]) => ({ desc, unit, min, max, rate: 0.21 }));

const HIGH_CATEGORIES = new Set(['bebidas_alcoholicas', 'vinos', 'cafe', 'limpieza']);

export function sampleBasket(rng, n = 8, category = null, mixIva = true) {
  let pool;
  if (mixIva) {
    pool = [...IVA_4, ...IVA_10, ...IVA_21];
  } else if (HIGH_CATEGORIES.has(category)) {
    pool = IVA_21;
  ...
  const items = rng.sample(pool, Math.min(n, pool.length));
  return items.map(p => {
    const unitPrice = Math.round(rng.uniform(p.min, p.max) * 100) / 100;
    const quantity = rng.choice([1, 2, 3, 4, 5, 6, 10, 12, 24, 0.5, 1.5]);
    return {
      description: p.desc,
      unit: p.unit,
