...
| `SENTRY_TRACES_SAMPLE_RATE` / `VITE_SENTRY_TRACES_SAMPLE_RATE` | `0.1` | Fraction (0-1) of requests sampled for performance traces in production (`NODE_ENV=production` server-side, `import.meta.env.PROD` client-side). Only applies in production — development always traces at `1.0` regardless of this variable. Empty, whitespace-only, non-numeric, or out-of-range values fall back to `0.1`. Does not affect error capture (`handleError` / `beforeSend`), which is unsampled. **Not symmetric at deploy time**: `SENTRY_TRACES_SAMPLE_RATE` is read from `process.env` on server start, so a Railway variable change takes effect on the next restart; `VITE_SENTRY_TRACES_SAMPLE_RATE` is a `VITE_`-prefixed var inlined into the client bundle at build time (via `import.meta.env`), so changing it only takes effect after a rebuild + redeploy — you cannot turn down client-side tracing during an incident without shipping a new build. |
...
| `STORAGE_CONNECT_TIMEOUT_MS` | `5000` | TCP connect timeout for the S3-compatible bucket (`STORAGE_DRIVER=railway`). Storage latency is unmeasured, so this is the AWS SDK's socket-level knob set to a conservative value, not a measured one (issue #999) |
| `STORAGE_TIMEOUT_MS` | `30000` | Per-attempt ceiling for a bucket read/write/delete, applied twice: as the SDK's `requestTimeout` and as the `withTimeout` wrapper that aborts the request. A hung read used to hold an extraction until pg-boss's 600 s `expireInSeconds` |
| `STORAGE_MAX_ATTEMPTS` | `3` | Attempts per bucket operation, 500 ms × 2ⁿ apart, transient failures only (5xx, 429, network, timeout — never a missing key or a 403). The S3 client's own `maxAttempts` is pinned to 1 so the two do not multiply; worst case is `STORAGE_MAX_ATTEMPTS × STORAGE_TIMEOUT_MS` plus backoff |
