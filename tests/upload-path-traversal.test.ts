...
vi.mock('$lib/server/batch', () => ({
	...
	getItem: vi.fn(async (id: string) => {
		if (id === '123e4567-e89b-12d3-a456-426614174000') {
			return { id: '123e4567-e89b-12d3-a456-426614174000', restaurantId: 'r1', displayName: 'factura.pdf', fileKey: 'uploads/factura.pdf' };
		}
		return null;
	}),
}));
...
describe('GET /api/upload/[id]/[file] path traversal guard', () => {
	...
	it('throws 400 Bad Request if params.id is not a valid UUID', async () => {
		const event = {
			params: { id: 'invalid-uuid', file: 'factura.pdf' },
			locals: {
				user: { id: 'u1' },
				restaurantId: 'r1',
			},
		...
		await expect(GET(event)).rejects.toMatchObject({ status: 400, body: { message: 'Invalid item ID' } });
	});
