...
describe.skipIf(!hasDbEnv)('saveReviewedInvoice → supplier contact fields (issue #385)', () => {
	...
	it('fills in missing contact fields on a later invoice without clobbering existing ones', async () => {
		...
		await saveReviewedInvoice(first, form(supplierName, 'INV-0001'), rid, UID);
		...
		await saveReviewedInvoice(second, form(supplierName, 'INV-0002'), rid, UID);
...
async function saveWithExtracted(supplier: string, invoiceNumber: string, extracted: Record<string, unknown>) {
	...
	return saveReviewedInvoice(item, form(supplier, invoiceNumber), rid, UID);
}
